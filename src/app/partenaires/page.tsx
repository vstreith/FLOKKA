...
const partners = [
  { name: 'Badminton Club de Barr', type: 'Club sportif', icon: Trophy, gradient: 'from-indigo-500 to-violet-500' },
  { name: 'FC Andlau', type: 'Club de football', icon: Trophy, gradient: 'from-violet-500 to-fuchsia-500' },
  { name: 'Association Les Vignes', type: 'Association locale', icon: Users, gradient: 'from-fuchsia-500 to-pink-500' },
  { name: 'Handball Obernai', type: 'Club sportif', icon: Trophy, gradient: 'from-amber-500 to-orange-500' },
  { name: "Comité des Fêtes d'Andlau", type: 'Association', icon: Users, gradient: 'from-cyan-500 to-blue-500' },
  { name: 'Menuiserie Schmitt', type: 'Petite entreprise', icon: Building2, gradient: 'from-pink-500 to-rose-500' },
]
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 pt-16">
        ...
        <section className="relative overflow-hidden mesh-bg py-24 px-4 sm:px-6 lg:px-8">
          <div className="pointer-events-none absolute -top-10 right-1/4 w-80 h-80 rounded-full bg-brand-cyan/20 blur-3xl animate-blob" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {partners.map((partner) => {
                ...
                return (
                  <div
                    key={partner.name}
                    className="group flex items-center gap-4 rounded-3xl bg-white p-6 ring-1 ring-violet-100 shadow-soft hover:shadow-card hover:-translate-y-1 transition-all duration-300"
                  >
                    <div
        ...
        <section className="px-4 sm:px-6 lg:px-8 pb-24 max-w-7xl mx-auto">
          <div className="relative overflow-hidden rounded-[2.5rem] bg-dark-gradient py-24 px-6 shadow-card">
            <div className="pointer-events-none absolute -top-16 left-1/3 w-72 h-72 rounded-full bg-brand-pink/30 blur-3xl" />
